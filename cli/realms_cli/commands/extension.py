...
def update_extension_imports(extension_id, action="add"):
    ...
    if action == "add":
        if import_line in content:
            console.print(f"[yellow]Import for {extension_id} already exists in extension_imports.py[/yellow]")
            return True
        ...
        with open(imports_file, "a") as f:
            f.write(import_line)
        console.print(f"[green]Added import for {extension_id} to extension_imports.py[/green]")
    
    elif action == "remove":
        if import_line not in content:
            console.print(f"[yellow]No import for {extension_id} found in extension_imports.py[/yellow]")
            return True
        ...
        with open(imports_file, "w") as f:
            f.write(new_content)
        console.print(f"[green]Removed import for {extension_id} from extension_imports.py[/green]")
...
def package_extension_command(extension_id: str, output_dir: Optional[str] = None):
    ...
    if not locations:
        console.print(f"[red]Extension '{extension_id}' not found or has no components[/red]")
        return False
            ...
            if "frontend_lib" in locations:
                ...
                for root, _, files in os.walk(frontend_lib_src):
                    for file in files:
                        ...
                        zipf.write(file_path, os.path.join(f"frontend/lib/extensions/{extension_id}", rel_path))
                        console.print(f"[blue]Added frontend/lib/extensions/{extension_id}/{rel_path}[/blue]")
            ...
            if "frontend_route" in locations:
                ...
                for root, _, files in os.walk(frontend_route_src):
                    for file in files:
                        ...
                        zipf.write(file_path, os.path.join(f"frontend/routes/(sidebar)/extensions/{extension_id}", rel_path))
                        console.print(f"[blue]Added frontend/routes/(sidebar)/extensions/{extension_id}/{rel_path}[/blue]")
            ...
            if "i18n" in locations:
                ...
                for root, _, files in os.walk(i18n_src):
                    for file in files:
                        ...
                        zipf.write(file_path, os.path.join(f"frontend/i18n/locales/extensions/{extension_id}", rel_path))
                        console.print(f"[blue]Added frontend/i18n/locales/extensions/{extension_id}/{rel_path}[/blue]")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if os.path.exists(backend_source) and os.listdir(backend_source):
            ...
            if not os.path.exists(init_file):
                ...
                console.print(f"[blue]Created __init__.py file for {extension_id}[/blue]")
        ...
        if os.path.exists(frontend_lib_source) and os.listdir(frontend_lib_source):
            ...
            for root, _, files in os.walk(frontend_lib_source):
                for file in files:
                    ...
                    os.makedirs(os.path.dirname(dst_file), exist_ok=True)
                    shutil.copy2(src_file, dst_file)
                    console.print(f"[blue]Installed frontend lib file: {rel_path}[/blue]")
            
            console.print(f"[green]Installed frontend lib files for {extension_id}[/green]")
        ...
        if os.path.exists(frontend_route_source) and os.listdir(frontend_route_source):
            ...
            for root, _, files in os.walk(frontend_route_source):
                for file in files:
                    ...
                    os.makedirs(os.path.dirname(dst_file), exist_ok=True)
                    shutil.copy2(src_file, dst_file)
                    console.print(f"[blue]Installed frontend route file: {rel_path}[/blue]")
            
            console.print(f"[green]Installed frontend route files for {extension_id}[/green]")
        
        i18n_source = os.path.join(temp_dir, "frontend/i18n/locales/extensions", extension_id)
        if os.path.exists(i18n_source) and os.listdir(i18n_source):
            i18n_target = os.path.join(paths["frontend_dir"], "src/lib/i18n/locales/extensions", extension_id)
...
def uninstall_extension_command(extension_id: str):
    ...
    if not locations and not any(os.path.exists(path) for path in paths.values()):
        console.print(f"[red]Extension {extension_id} not found or already uninstalled[/red]")
        return False
    ...
    if "frontend_lib" in locations or os.path.exists(paths["frontend_lib"]):
        try:
            if os.path.exists(paths["frontend_lib"]):
                ...
                console.print(f"[green]Removed frontend library files for {extension_id}[/green]")
        except Exception as e:
    ...
    if "frontend_route" in locations or os.path.exists(paths["frontend_route"]):
        try:
            if os.path.exists(paths["frontend_route"]):
                ...
                console.print(f"[green]Removed frontend route files for {extension_id}[/green]")
        except Exception as e:
    ...
    if "frontend_custom_routes" in locations:
        try:
            ...
            console.print(f"[green]Removed custom route files for {extension_id}[/green]")
        except Exception as e:
...
def extension_command(
    ...
    if action == "list":
        ...
        if not extension_id:
            console.print("[red]Error: --extension-id is required for package action[/red]")
            raise typer.Exit(1)
        package_extension_command(extension_id)
    elif action == "install":
        if not package_path:
            console.print("[red]Error: --package-path is required for install action[/red]")
            raise typer.Exit(1)
        install_extension_command(package_path)
    elif action == "uninstall":
        if not extension_id:
            console.print("[red]Error: --extension-id is required for uninstall action[/red]")
            raise typer.Exit(1)
        uninstall_extension_command(extension_id)
    else:
        console.print(f"[red]Unknown action: {action}[/red]")
        console.print("[yellow]Available actions: list, install-from-source, package, install, uninstall[/yellow]")
        raise typer.Exit(1)
