...
def create_proposal(args: str) -> str:
    ...
    try:
        ...
        return json.dumps({"success": True, "proposal_id": proposal.id, "proposal": proposal.to_dict()})
    except Exception as e:
...
def delete_proposal(args: str) -> str:
    ...
    try:
        ...
        return json.dumps({"success": True, "message": "Proposal cancelled successfully"})
    except Exception as e:
...
def vote_on_proposal(args: str) -> str:
    ...
    try:
        ...
        if not all([proposal_id, vote_value, voter_id]):
            return json.dumps({"success": False, "error": "proposal_id, vote, and voter_id are required"})
        
        if vote_value not in ["yes", "no", "abstain", "null"]:
            return json.dumps({"success": False, "error": "Invalid vote value. Must be yes, no, abstain, or null"})
        ...
        if current_metadata.get("status") != "voting":
            return json.dumps({"success": False, "error": "Proposal is not open for voting"})
        ...
        vote_counts = current_metadata.get("vote_counts", {"yes": 0, "no": 0, "abstain": 0, "null": 0})
        ...
        return json.dumps({"success": True, "vote_id": vote.id, "updated_counts": vote_counts})
    except Exception as e:
        ...
        for vote in votes:
            ...
            if vote_metadata.get("proposal_id") == proposal_id:
                proposal_votes.append({
                    ...
                    "created_at": vote.created_at if hasattr(vote, 'created_at') else None
                })
