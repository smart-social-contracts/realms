    ...
    if isinstance(expected, dict) and isinstance(actual, dict):
        for key, expected_value in expected.items():
            ...
            if not compare_json(expected_value, actual[key], route + "/%s" % key):
                print(
                    f"[{route}] Mismatch found for key '{key}': Expected {expected_value}, got {actual[key]}"
                )
                return False
        return True
        ...
        if len(expected) != len(actual):
            print(
                f"[{route}] List length mismatch: Expected {len(expected)}, got {len(actual)}"
            )
            return False
        for i, (expected_item, actual_item) in enumerate(zip(expected, actual)):
            if not compare_json(expected_item, actual_item, route + "/%d" % i):
                print(
                    f"[{route}] Mismatch found in list at index {i}: Expected {expected_item}, got {actual_item}"
                )
                return False
        return True
    else:
