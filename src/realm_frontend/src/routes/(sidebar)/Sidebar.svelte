<script lang="ts">
	...
	const coreNavItems: NavItemWithHref[] = [
		...
		{ name: $_('navigation.settings') || 'Settings', icon: CogOutline, href: '/settings' } // For all users
	];

	// Filter core navigation items based on user profiles
	$: filteredCoreNavItems = coreNavItems.filter(item => {
		// If no profiles are available, only show basic items
		if (!$userProfiles || $userProfiles.length === 0) {
			return true; // Show basic navigation items
		}
		
		// Show all core items to authenticated users
		return true;
	});
	...
	$: extensionsByCategory = (() => {
		...
		filteredExtensions.forEach(ext => {
			// Get categories from manifest, default to 'general' if none specified
			const extCategories = ext.categories || ['general'];
	...
	const categoryNames: Record<string, string> = {
		...
		'general': $_('categories.general') || 'General'
	};
