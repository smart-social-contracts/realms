<script lang="ts">
  ...
  let item: CodexListing | null = null;
  let loading = true;
  let error = '';
  ...
  async function load(codexId: string) {
    loading = true;
    error = '';
    try {
  ...
  async function refreshLikes(_a: boolean, codexId: string) {
  ...
  async function refreshPurchased(_a: boolean, principal: string | null, codexId: string) {
  ...
  async function doDelist() {
    if (!item) return;
    if (!confirm(`Delist '${item.codex_id}'? It will be removed from listings and rankings.`)) return;
    try {
      await marketplaceClient.delistCodex(item.codex_id);
      auditMsg = '✅ Delisted.';
      item = null;
      setTimeout(() => { window.location.href = '/codices'; }, 800);
    } catch (e: any) {
...
<a class="back" href="/codices">← Back to codices</a>
    ...
    <header>
      ...
      <div class="title-block">
        ...
        <p class="meta">
          ...
          {#if item.realm_type} · realm: <code>{item.realm_type}</code>{/if}
        </p>
        <div class="badges">
    ...
    {#if isOwner()}
      <section class="block owner">
        ...
        <div class="owner-actions">
          ...
          <a class="btn" href={`/upload?prefill=${encodeURIComponent(item.codex_id)}`}>Edit / new version</a>
