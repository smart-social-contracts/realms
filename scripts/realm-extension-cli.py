...
def install_all_packages(generate_manifests=True):
    ...
    # Generate extension manifests after successful installation
    if success and generate_manifests:
        paths = get_project_paths()
        extensions_source_dir = os.path.join(paths["project_root"], "extensions")
        if os.path.exists(extensions_source_dir):
            if not generate_extension_manifests("extensions"):
                log_warning("Failed to generate extension manifests, but packages were installed successfully")
        else:
            log_warning("Extensions source directory not found, skipping manifest generation")
...
def install_from_source(source_dir="extensions"):
    ...
    try:
        ...
        # Step 3: Generate extension manifests for Kybra canister
        if not generate_extension_manifests(source_dir):
            log_error("Failed to generate extension manifests")
            return False
...
def generate_extension_manifests(source_dir="extensions", output_path=None):
    """Generate extension_manifests.py file from source extension manifests"""
    paths = get_project_paths()
    extensions_source_dir = os.path.join(paths["project_root"], source_dir)
    ...
    if output_path is None:
        output_path = os.path.join(paths["backend_dir"], "extension_packages", "extension_manifests.py")
    
    log_info(f"Generating extension manifests from: {extensions_source_dir}")
    log_info(f"Output file: {output_path}")
    
    # Collect all extension manifests
    extension_manifests = {}
            ...
            if os.path.exists(manifest_path):
                try:
                    with open(manifest_path, 'r', encoding='utf-8') as f:
                        ...
                        extension_manifests[item] = manifest_data
                        log_info(f"Loaded manifest for extension: {item}")
                except json.JSONDecodeError as e:
                    log_error(f"Invalid JSON in manifest for {item}: {str(e)}")
                    continue
                except Exception as e:
                    log_error(f"Error loading manifest for {item}: {str(e)}")
                    continue
    
    if not extension_manifests:
        log_warning("No extension manifests found")
        return False
...
This file is auto-generated by scripts/realm-extension-cli.py
...
def get_extension_manifest(extension_name: str) -> dict:
    """Get manifest for a specific extension"""
    return EXTENSION_MANIFESTS.get(extension_name, {})
...
def list_extension_names() -> list:
    """Get list of all extension names"""
    return list(EXTENSION_MANIFESTS.keys())
'''
    ...
    try:
        ...
        log_error(f"Error writing extension_manifests.py: {str(e)}")
        return False
...
def main():
    ...
    generate_parser = subparsers.add_parser("generate-manifests", help="Generate extension_manifests.py from source extensions")
    generate_parser.add_argument("--source-dir", help="Source directory containing extensions", default="extensions")
    generate_parser.add_argument("--output-path", help="Output path for extension_manifests.py", default=None)
    ...
    if args.command == "list":
        ...
        if not generate_extension_manifests(args.source_dir, args.output_path):
            return 1
