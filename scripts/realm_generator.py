...
class RealmGenerator:
    ...
    def generate_humans(self, users: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for user in users[1:]:  # Skip system user
            first_name = user["id"].split("_")[0].title()
            last_name = user["id"].split("_")[1].title()
...
def main():
    ...
    summary = {
        ...
        "statistics": {
            "total_users": len(realm_data['users']),
            "total_organizations": len(realm_data['organizations']),
            "total_transactions": len(realm_data['transfers']),
            "total_disputes": len(realm_data['disputes']),
            "total_mandates": len(realm_data['mandates'])
        }
    }
