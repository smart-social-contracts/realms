    ...
    for shared_canister in $SHARED_CANISTERS; do
        ...
        if [ -n "$existing_id" ]; then
            ...
            if [[ "$shared_canister" == *ckbtc_indexer ]] || [ "$shared_canister" = "ckbtc_indexer" ]; then
                ...
                # icp deploy doesn't support --argument; use deploy + install with --args
                retry_icp icp deploy "$shared_canister" --yes || true
                retry_icp icp canister install "$shared_canister" --yes --args "(opt variant { Init = record { ledger_id = principal \"$ledger_id\" } })"
            else
                # init_args are read from icp.yaml (generated by generate_icp_yaml.py)
