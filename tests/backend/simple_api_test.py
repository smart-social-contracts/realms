...
from realm_backend.api.extensions import list_extensions, call_extension_api
...
async def test_list_extensions():
    ...
    try:
        ...
        assert hasattr(first_ext, "description"), "Extension missing 'description' field"
        assert hasattr(first_ext, "required_permissions"), "Extension missing 'required_permissions' field"
        assert hasattr(first_ext, "granted_permissions"), "Extension missing 'granted_permissions' field"
        assert hasattr(first_ext, "entry_points"), "Extension missing 'entry_points' field"
...
async def test_call_extension_api():
    ...
    try:
        ...
        assert result.success, f"API call failed: {getattr(result, 'error', 'Unknown error')}"
...
async def test_direct_call_extension():
    ...
    try:
        ...
        result = await call_extension("test_extension", "test_function", value="direct-test")
        
        # Verify the result
        assert "result" in result, "Result missing expected field"
        assert result["result"] == "Processed: direct-test", f"Unexpected result: {result['result']}"
