...
def test_generic_helper_function():
    ...
    try:
        ...
        assert_in(content, "def _generic_paginated_list(entity_name: str, list_function, page_num: nat, page_size: nat, record_class, data_key: str)")
        print_success("Generic helper function has correct signature")
        
        entity_functions = [
            "get_users", "get_mandates", "get_tasks", "get_transfers",
            "get_instruments", "get_codexes", "get_organizations", 
            "get_disputes", "get_licenses", "get_realms", "get_trades",
            "get_proposals", "get_votes"
        ]
        
        for func in entity_functions:
            assert_in(content, f"def {func}(page_num: nat, page_size: nat) -> RealmResponse:")
