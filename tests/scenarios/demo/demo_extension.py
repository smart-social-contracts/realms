...
def initialize_state():
            ...
            "features": [
                {
                    ...
                    "geometry": {
                        ...
                        "coordinates": [
                            [
                                [
                                    [8.617437378000091, 47.757318624000078],
                                    [8.629839721000053, 47.762796326000085],
                                    [8.635007365000149, 47.784603780000083],
                                    [8.644102417000113, 47.791011658000031],
                                ]
                            ]
                        ],
                    },
    ...
    land_token.mint(state.wallet.get_address(land_token), 1000, metadata={})
    ...
    for i in range(NUM_CITIZENS):
        ...
        base_income = MIN_DAILY_INCOME + (MAX_DAILY_INCOME - MIN_DAILY_INCOME) * (
            i / NUM_CITIZENS
        )
...
def simulate_daily_activities(citizen, coin, config, day_number):
    ...
    expense_ratio = (
        0.6 + math.sin(day_number / 30) * 0.1
    )  # Varies between 50-70% over monthly cycle
    daily_expense = daily_income * expense_ratio
    daily_expense = max(
        config["MIN_DAILY_EXPENSE"], min(config["MAX_DAILY_EXPENSE"], daily_expense)
    )
...
""".strip()
now = get_system_time()
proposal = ggg.Proposal.new("Proposal to increase budget", proposal_code, now)
