...
def test_extensions():
    ...
    run_command("realms extension install-from-source")
    ...
    for name in EXTENSION_NAMES:
        run_command(f"realms extension package --extension-id {name}")
    ...
    for name in EXTENSION_NAMES:
        run_command(f"realms extension install --package-path {name}.zip")
