...
def get_extension_names():
    ...
    return [
        name for name in os.listdir(extensions_dir)
...
def test_extensions():
    ...
    print(f"Found {len(EXTENSION_NAMES)} extensions: {', '.join(sorted(EXTENSION_NAMES))}")
    ...
    for name in EXTENSION_NAMES:
        ...
        run_command(f"realms extension package --extension-id {name} --source-dir {ext_source} --package-path {output_path}")
