...
from .config import NUM_INSTRUMENTS, NUM_TRADES, NUM_TRANSFERS, NUM_USERS
...
def run():
    ...
    for i in range(NUM_TRADES):
        ...
        trade = Trade(metadata=f'{{"type": "{trade_type}", "id": "TR{i:06d}"}}')
        trades.append(trade)
    ...
    logger.info(
        f"Created {len(trades)} trades, {len(transfers)} transfers, and {len(disputes)} disputes"
    )
