...
def run(batch=None):
    ...
    logger.info(
        f"Creating {NUM_PROPOSALS} governance proposals with {NUM_VOTES_PER_PROPOSAL} votes each"
    )
    ...
    for i in range(NUM_PROPOSALS):
        ...
        proposal = Proposal(name=topic["title"], metadata=json.dumps(proposal_metadata))
        proposals.append(proposal)

        # Generate votes for this proposal
        proposal_votes = generate_realistic_votes(
            f"proposal_{i}", NUM_VOTES_PER_PROPOSAL
        )
