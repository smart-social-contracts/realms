...
function SiteNav({ active = 'home' }) {
  ...
  const mobileNavLinkClass = (page) =>
    page === active ? 'block text-primary-900 font-medium' : 'block text-slate-600 hover:text-slate-900'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-8">
            <a href={sectionHref('#features')} className={navLinkClass('home')}>
              {t('nav.howItWorks')}
            </a>
            <Link to="/marketplace" className={navLinkClass('marketplace')}>
              {t('nav.marketplace')}
            </Link>
            <a href={sectionHref('#forpeople')} className={navLinkClass('home')}>
              {t('nav.forPeople')}
            </a>
            <a href={sectionHref('#forinstitutions')} className={navLinkClass('home')}>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden glass-dark border-t border-slate-200">
          <div className="px-4 py-4 space-y-4">
            <a href={sectionHref('#features')} className={mobileNavLinkClass('home')}>
              {t('nav.howItWorks')}
            </a>
            <Link to="/marketplace" className={mobileNavLinkClass('marketplace')} onClick={() => setMobileMenuOpen(false)}>
              {t('nav.marketplace')}
            </Link>
            <a href={sectionHref('#forpeople')} className={mobileNavLinkClass('home')}>
              {t('nav.forPeople')}
            </a>
            <a href={sectionHref('#forinstitutions')} className={mobileNavLinkClass('home')}>
              {t('nav.forInstitutions')}
            </a>
            <a
              href="https://github.com/smart-social-contracts/realms/blob/main/ROADMAP.md"
