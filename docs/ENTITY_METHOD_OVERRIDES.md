...
vault
  yield treasury.send(ic.id().to_str(), 1) => vault.execute_transfer
  yield treasury.refresh() => vault.refresh_balance

  realm manifest.json to configure the overrides
  {
    "entity": "Transfer",
    "method": "execute",
    "implementation": "methods.execute_transfer"
  }
  code in vault implements the ggg method override


user registration => create new Invoice for a Service
  User.user_register_posthook
  upload codex with the code

Invoice paid => service status for the user is Active
  (todo implement)
...
1- vault manifest overrides the treasury send and refresh, the code is in the vault repo
2- user registration can be done using the manifest override of the User create hook, which calls a codex that creates a new invoice. this code is in the realm installation package and it is installed as a codex when the realm is created. the same for the Invoice update hook, which updates the user's service status when the invoice is paid.


So, the manifest is the way to configure these overrides at the realm level.
The overriding code can be set either in the extension package or as a separate codex deployed with the realm.
